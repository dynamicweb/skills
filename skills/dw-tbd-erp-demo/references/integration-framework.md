...
- DW's `BCSetupUpdateProvider` populates default settings (`indexBuildKey`, `buildName`, `workflowStateId`) that have to be corrected per-demo -- see [`../dw-tbd-pim-for-bc/references/dynamicweb-connector-settings.md`](../../dw-tbd-pim-for-bc/references/dynamicweb-connector-settings.md).
...
- Live flavor (ngrok + AppStore connector + `/admin/api/BC*`): [`../dw-tbd-pim-for-bc/SKILL.md`](../../dw-tbd-pim-for-bc/SKILL.md).
- Channel-vs-shop pedagogy beat (ShopType enum, why ShopType=3 is feeds not ERPs): [`../dw-tbd-pim-demo/references/structural-model.md`](../../dw-tbd-pim-demo/references/structural-model.md) Â§2.1.
