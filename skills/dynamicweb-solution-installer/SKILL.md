---
name: dynamicweb-solution-installer
description: >
  Installs DynamicWeb Swift 2 from scratch. Downloads the latest Swift 2 database, files,
  and demo data from doc.dynamicweb.com, imports the database via sqlpackage, extracts
  files, installs the temporary Custom.Mcp add-ins payload, writes a first-run bootstrap
  manifest, and writes GlobalSettings.Database.config. Use when the user has a fresh or empty
  Dynamicweb instance and needs Swift 2 installed.
---
