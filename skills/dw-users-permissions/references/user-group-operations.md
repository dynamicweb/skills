...
| Create / edit a user group | `save_user_groups` | `GroupSave`, a full-model save (`UserGroupSave` does not exist, and `UserGroupById` / `UserGroups` answer `Unknown query`) | `save_user_groups` has no type member, so the user-and-group type (`SystemAccount` for a B2B account group) is set on the group's admin screen in product, or through `GroupSave` out of product (`recipes-users.md` "Set a user group's type"). |
