...
| Build the demo cheat-sheet page (canonical four-call MCP chain + customer-safety rules) | references/cheat-sheet.md |
...
| **Fresh-deserialize fiction sweep (run BEFORE the demo path is built)** — the prospect-visible copy a shipped baseline serves that is false about the customer: vendor meta description and privacy policy, invented statistics, foreign-industry legal boilerplate, a placeholder phone and a promised live chat, chrome links to empty states; plus the template-fallback family that lives in an ATTRIBUTE (blanking a field can RESTORE the vendor word), the default-currency symbol/culture/rate check, and retiring a licence-gated surface rather than demoing a dead button. Triggers: "rebrand the site", "is anything still saying the vendor's name", a page reachable only from the footer | **references/fresh-deserialize-sweep.md** |
| **Warranty / service / claims surface** — rename the platform's RMA machine instead of modelling a parallel claim entity (the comment trail, the backend transition screen and the claim numbering come free); net-new list/detail template copies repointed through the paragraph's module settings; the serial column that exists with no frontend input; and the shipped `<BoughtFromDate>` that silently empties the claim form's order picker. Triggers: warranty claims, returns/RMA vocabulary, "the claim form shows no orders" | **references/rma-claims.md** |
...
| **B2B DC pattern** (one AccessUser group per Stock Location → unlocks DC-scoped Assortments + Shipping methods + Shipping fees; the canonical DW10 mechanic for any multi-DC wholesale demo; vendor-blessed; the anonymous price-leak assert; cart automation probes) | references/customer-center.md §11 |
...
| **Choose the branding path BEFORE the first brand write**: YAML brand layer first (the default), deserialize-then-tools, or tools only; the measured cost of each and the four conditions under which the tool path is right | **[`../dw-demo-base/references/branded-demo-paths.md`](../dw-demo-base/references/branded-demo-paths.md)** |
...
| **A page passes every content assert and still looks or behaves wrong** — horizontal-overflow diagnosis (three numbers, offender by right edge, both auth states), the visually-hidden containment rule, hit-testing a stretched-link card, contrast | [`layout-verification.md`](../dw-swift-building/references/layout-verification.md) |
| **Grid-row mechanics** — what `GridRowCopy` carries from the donor and how to normalise it, the column-binding law, the spacing an inert row still pays, idempotent row minting | [`grid-rows-and-binding.md`](../dw-swift-building/references/grid-rows-and-binding.md) |
| **A shipped Swift 2 template is the defect** — templates that do not compile on DW 10.29/.NET 10, the search dropdown that crashes without product images, order surfaces that omit the payment method, the price a "price-hidden" page still delivers in markup, the Express Buy feed's row contract | [`shipped-template-defects.md`](../dw-swift-building/references/shipped-template-defects.md) |
