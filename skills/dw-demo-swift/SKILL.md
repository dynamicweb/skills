...
**Base-layer shape — scaffolding-only.** The `base` layer is a `config/replace/merge` tree that ships the Swift **scaffolding + starter content only — no sample catalog**: `replace/` ships framework `_sql/` (currencies, countries, languages, shops, payments, shippings, VAT) plus content (areas, pages, starter content structure). It ships **zero EcomProducts / EcomGroups / EcomPrices** — a `base` deserialize lands framework + pages + starter content and an **EMPTY catalog** by design. The catalog comes from the `sample-data` layer (SQL files in `merge/_sql/`, activated when an edition sets `sampleData: true` — e.g. the `swift-demo` edition) or is authored **per-demo** via the PIM modelling recipes — route it to [`../dw-demo-pim/SKILL.md`](../dw-demo-pim/SKILL.md), do not expect the base to supply products (that is the whole point: each demo tailors its own catalog, not a pre-baked store). The framework rows still ship with the base layer, so a fresh DB no longer needs the framework pre-seeded, unlike the retired content-only `Swift2.2` baselines. The area YAML uses string FKs (`AreaEcomShopId: "SHOP1"`, `AreaEcomCountryCode: "DE"`) that resolve against the framework rows the replace pass lands. Caution: framework rows travel in `replace`, which is **source-wins** — on a host with hand-curated SHOP1/DE/EUR/LANG1 rows the base layer UPDATES those rows. See [`references/deserialize-flow.md`](references/deserialize-flow.md) §3 for the full shape contract and the PIM-curated-host warning.
...
|---|---|
| **Load the Swift `base` layer content into the host** (POST `/Admin/Api/SerializerDeserialize`; includes the design-package deploy step) | **references/deserialize-flow.md** |
...
| **Header menu: make it read as a menu** — why a fresh bar is flat (childless top nodes), the `save_groups` nav-depth recipe, the `theme-nav-polish` shared default, and the three interaction platform-truths (Popper-gap bridge, `::before`=icon/`::after`=underline caret collision, dropdown `min-width`); opt-in `data-nav-icon` icons | references/header-menu.md |
...
| Drop per-demo Style assets by hand (Color Schemes / Buttons / Typography / Fonts JSON+CSS pairs in `Files/System/Styles/`; Area row wiring; theme layers from the Distribution, checked out per-demo into `<demo-root>\distribution\layers\theme-<name>\`) | references/styles-assets.md |
