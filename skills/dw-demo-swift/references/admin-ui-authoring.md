...
2. **Translate mood board into Style tools.** Admin UI:
   - Settings → Content → Styles → **Color Schemes** (primary palette + scheme variants)
   - Settings → Content → Styles → **Typography** (font family, scale, weights)
   - Settings → Content → Styles → **Buttons** (shape, padding, hover states)
3. **Upload assets via Assets manager** — logo, favicon, hero imagery into `Files/Images/branding/` per [asset-organisation.md](asset-organisation.md).
4. **Connect styles to website settings.** Admin UI:
   - Website Settings → **Layout** tab → bind the new color scheme + typography
   - Content section → Design section → **Favicon** → upload + bind
5. **Build layout in Visual Editor.** Admin UI: Pages → page-preset → grid row → paragraphs. See the Visual Editor sections below and [templates.md](templates.md).

The configuration-only doc is candid that the approach "will consequently not be able to do everything" — if the customer's brand requires something the style tools can't express, escalate per [re-skin.md](re-skin.md), don't fight the configuration surface.

## Visual Editor surfaces (where it lives in admin UI)

- **Pages** → any page-preset or content page → grid row → paragraph: clicking a paragraph opens its property panel; that panel IS the Visual Editor for that paragraph type.
- **Settings** → **Areas** → **SHOP1** → **Site Settings**: site-level affordances (title, default language, fallback layout).
- **Pages** → **Theme** (the `Theme` page-preset): theme-tokens-as-paragraph-properties; the standard re-skin entrypoint per [re-skin.md](re-skin.md).

The Style-tools admin paths in workflow step 2 + 4 above are the same forms the Visual Editor theme work uses — one set of paths, stated once.

## What the Visual Editor covers, and the escalation per gap

| Change | Visual Editor | Escalation if VE cannot do it |
|--------|---------------|------------------------------|
| Theme tokens (colors, typography) | YES (Style tools paths in step 2; bind via Website Settings → Layout) | `<customer>_custom.css` overrides consuming `--dw-*` variables ([re-skin.md](re-skin.md) — never the stock `custom.css`) |
| Logo / hero image swap | YES (paragraph property → asset file path) | n/a (VE handles fully) |
...
| Data-shape transformation, conditional rendering, external calls / business logic | NO | Controller / provider `.cs` — triggers base's customisations-ledger preflight (Approve+log / Refactor / Cancel). See [dynamicweb-demo-base/references/customisations.md](../../dw-demo-base/references/customisations.md). |

**Razor `.cshtml` is NOT in the customisations-ledger glob.** The right-column escalations do not all trigger the preflight — only `.cs` controllers / providers do. Per [dynamicweb-demo-base/references/customisations.md](../../dw-demo-base/references/customisations.md) §5, Razor content layouts are conventional, normal demo-build flow per the Swift docs ([doc.dynamicweb.dev/swift/design/pixel-perfect.html](https://doc.dynamicweb.dev/swift/design/pixel-perfect.html)).
...
- `Dynamicweb.Host.Suite/wwwroot/Files/Templates/Designs/Swift-v2/<paragraph-instance-config>.json` (paragraph property persistence — these are config, not Razor)
- `Files/System/Styles/{ColorSchemes,Buttons,Typography}/*.{json,css}` (Style-tools saves — see [styles-assets.md](styles-assets.md))

You should NEVER see:
- `*.cs` changes in `Controllers/` or `Providers/` (would mean a custom controller was added — triggers the customisations-ledger preflight in base)
- `*.scss` / `*.ts` changes (would mean asset-pipeline source was touched — recompilation drift)
- Changes to any file named exactly `custom.css` (Swift-shipped sample code — brand CSS belongs in `<customer>_custom.css`; hard rule in [re-skin.md](re-skin.md) §What NOT to touch, audit grep #9 in [dw10-canonical-surfaces.md](dw10-canonical-surfaces.md))

If `*.cshtml` changes appear, that is **not automatically a preflight hit** (Razor-glob note above). New content-layout `.cshtml` files are part of the normal escalation ladder ([re-skin.md](re-skin.md) §Pixel-perfect escalation); modifications to existing standard `.cshtml` templates under `Files/Templates/Designs/Swift-v2/` or `Files/Templates/Paragraph/` are the thing to avoid — per the pixel-perfect doc quoted in [re-skin.md](re-skin.md) §Pixel-perfect escalation: extend or create new templates, never modify standard ones.
