...
**The mirror image: with the base href suppressed, Swift's variant selector 404s.** `DoNotIncludeBaseHref` is a `GlobalSettings` frontend flag whose DW default is `False` (base href emitted); several demo baselines and the clones taken from them ship it `True`, which is why this reaches a demo that nobody changed. Swift builds the variant-selector URL **relative** (`Default.aspx?ID=<shop page>&GroupID=…&ProductID=…` in `Components/VariantSelector.cshtml`) and relies on the base href to resolve it at the site root. With no base href it resolves under the current page path, and DW answers that with `404`: clicking an option leaves price, SKU and image on the master, keeps add-to-cart disabled, and rewrites the address bar to a URL that 404s on refresh. Only a root-level `/Default.aspx?…` resolves. **Leave the setting at the DW default rather than patching the template** — an absolute URL in `VariantSelector.cshtml` is a Razor customisation on every demo, and the setting is what Swift expects. Verify both halves in one pass, because restoring the base href brings the bare-fragment trap above back: on a PDP with two or more variant options, click a non-default option and assert no `4xx`, the SKU text changing to the variant's own number and add-to-cart enabled; and assert the home page renders a `<base href>` at all. **A beat or demo script that deep-links with `&VariantID=` in the URL hides this completely** — the assertion has to be a click.
