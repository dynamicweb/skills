...
1. **Measure the canvas, not the viewport.** `overflow-x: hidden` on `body` hides horizontal stretch from naive probes — a stretched document reads as "fine" to a viewport check. Probe **`document.body.scrollWidth`** (NOT `document.documentElement.scrollWidth` — the `<html>` element is clipped by the body's `overflow-x`) at **390** and **1440**; pass = exactly viewport-wide (`body.scrollWidth <= innerWidth`) **and `innerWidth` equal to the width you asked for** (the second half is what catches a layout viewport widened by unshrinkable content). On marine the canvas was **1356px at 390** and the symptom read as "missing PLP images" — it was a stretched canvas with broken lazy-load painting in the blank right margin, not a lazy-load bug.
...
- **Assert `document.body.scrollWidth <= innerWidth` at 390 AND `innerWidth === 390`** (body, not documentElement — `overflow-x: hidden` masks the stretch otherwise). Both numbers are the canvas-fit bar: unshrinkable content widens the layout viewport, and once it has, the first comparison passes by construction on a page that renders zoomed out — see [`../../dw-demo-base/references/visual-qa.md`](../../dw-demo-base/references/visual-qa.md) "Breakpoints".
