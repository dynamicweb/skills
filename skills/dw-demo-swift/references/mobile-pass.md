...
**The pass needs a browser runner, and a demo build machine has one — find it before concluding otherwise.** Every measurement below reads the live DOM at a device descriptor. The driver ships with the demo agent tooling, not under the skills tree, so a search of the skills folder finds nothing and means nothing; resolve its location from the demo agent's configuration ([`dw-demo-base/references/visual-qa.md`](../../dw-demo-base/references/visual-qa.md) "Driving the instrument"). Only a lookup that genuinely finds no driver makes the viewport leg **UNRUNNABLE**, reported with the failed lookup named. A gate run that stamps PASS on an unmeasured viewport leg is the defect the leg exists to catch ([`re-skin.md`](re-skin.md) "An assert that cannot fail is not an assert").
