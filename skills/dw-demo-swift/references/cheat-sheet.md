...
1. **`save_pages`** — `id=0, areaId=<area>, urlName='demo', menuText='Demo Info', active=true, hidden=false, navigationTag='DemoInfo', sort=1000, metaTitle='Demo information'`. Capture the new page id. **Product limit:** `urlName`, `navigationTag` and the visibility members ARE members of the `save_pages` input schema on MCP 0.4.4, each with its own description — they are accepted and then **not persisted** (verified 10.27.x-10.28.x), so the page lands with a slug derived from `menuText` and default visibility no matter what you pass, and the response echoes what you sent. Do not go looking for a better-named tool; there is none. In product, work with the limit rather than around it: **make `menuText` produce the slug you want** (`menuText='Demo'` yields `/demo`), keep `hidden=false` and rely on the `sort=1000` mechanic below for nav exclusion, and drive menu placement with `set_page_menu` instead of the tag column. Read the page back with `get_pages_by_ids` and use the slug it actually has when writing any cross-link.

Pinning a slug `menuText` cannot produce is genuinely out of product. **Out-of-product fallback, labelled as one:** the column writes for `PageUrlName`, the navigation tag and `PageHidden` live in [`dw-data-access`](../../dw-data-access/SKILL.md) `recipes-content.md` §"Pin a page slug". They are a last resort under the retirement notice in [`sql-direct-seeding.md`](sql-direct-seeding.md), not a default step: never reach for them because the API got hard, only because the API provably cannot express the result.
...
4. **`set_paragraph_item_fields`** — on the new paragraph id, in ONE call: `Text` = the cheat-sheet HTML (Bootstrap `table.table-bordered` + a `card` grid for the facts panel both work without custom CSS), `Title` and `Subtitle` = `""`, and `FirstButton` and `SecondButton` = `""`. Vertical spacing in `Text` must be markup — a `RichTextEditor` field drops empty lines on write, `<pre>` included ([`dw-content-modelling/references/page-paragraph-writes.md`](../../dw-content-modelling/references/page-paragraph-writes.md)).
