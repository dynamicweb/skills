...
1. **`save_pages`** — `id=0, areaId=<area>, urlName='demo', menuText='Demo Info', active=true, hidden=false, navigationTag='DemoInfo', sort=1000, metaTitle='Demo information'`. Capture the new page id. **Product limit:** `save_pages` does **not** carry `urlName`, the navigation tag, or `hidden` (verified 10.27.x-10.28.x) — the page lands with a slug derived from `menuText` and default visibility no matter what you pass, and the response echoes what you sent. In product, work with the limit rather than around it: **make `menuText` produce the slug you want** (`menuText='Demo'` yields `/demo`), keep `hidden=false` and rely on the `sort=1000` mechanic below for nav exclusion, and drive menu placement with `set_page_menu` instead of the tag column. Read the page back with `get_pages_by_ids` and use the slug it actually has when writing any cross-link.
