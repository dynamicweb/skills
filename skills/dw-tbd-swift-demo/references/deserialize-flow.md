...
- [`../../dw-tbd-demo-base/references/setup-checks.md`](../../dw-tbd-demo-base/references/setup-checks.md) is green (DW_VAULT, NODE_TLS_REJECT_UNAUTHORIZED, .NET SDK, ProjectTemplates, SQL Express, vault slot inventory all probed and resolved).
- [`../../dw-tbd-demo-base/references/scaffold.md`](../../dw-tbd-demo-base/references/scaffold.md) produced a running `Dynamicweb.Host.Suite` (port reachable, host responds at `/admin`).
- [`../../dw-tbd-demo-base/references/mcp-setup.md`](../../dw-tbd-demo-base/references/mcp-setup.md) verification gate passed (`claude mcp list` shows `dynamicweb-commerce-mcp âœ“ Connected` AND in-conversation `ToolSearch +dynamicweb` returns >200 tools).
- **DynamicWeb.Serializer is installed in the host** per [`../../dw-tbd-demo-base/references/serializer-reference.md`](../../dw-tbd-demo-base/references/serializer-reference.md) "Installation" section (DLL built + copied to `bin/Debug/net10.0/`, `Files/Serializer.config.json` staged, host restarted). This is a one-time-per-host step.
...
Use the same snippet documented in [`../../dw-tbd-demo-base/references/mcp-setup.md`](../../dw-tbd-demo-base/references/mcp-setup.md) Section 1 (port-discovery from `Dynamicweb.Host.Suite/Properties/launchSettings.json`). That reference is the single source of truth for port discovery; do not duplicate it here.
...
For internals (where each category is detected in source), see [`../../dw-tbd-demo-base/references/serializer-reference.md`](../../dw-tbd-demo-base/references/serializer-reference.md).
