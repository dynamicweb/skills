...
| [references/order-states-and-quotes.md](references/order-states-and-quotes.md) | building a state ladder, the id-less create that overwrites a state when `EcomNumbers` lags, the re-issued state id and dangling transition rows, `OrderCancel` + `OrderDelete`, `UpdateCartToQuote` / `DowngradeToCart`, the dead Accept-quote button |
