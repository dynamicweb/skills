...
**Platform floor — the package installs on Dynamicweb 10.17.5 or newer.** The app-store install filter compares the host's DW version against the `DynamicwebVersion` the package was packed at, and the engine's release workflow packs at 10.17.5 (below it the Alert/AlertType admin-UI APIs are missing), so every host from 10.17.5 up is offered the package; the csproj's own default pin is only the local build's binding and is not the install floor. That is where it installs, not where it is proven: the engine is proven on the hosting ring its README names, and the Distribution's layers carry their own higher DW floor. Serializer endpoints that 404 on a host at or above 10.17.5 are a missing install or config, not the platform floor.
