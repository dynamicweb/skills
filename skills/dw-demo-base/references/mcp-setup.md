...
**Why not `NODE_EXTRA_CA_CERTS` for a LOCALHOST dev cert?** The ASP.NET dev cert is a self-signed leaf, not a CA. `NODE_EXTRA_CA_CERTS` only adds **trusted CAs**, and Node will not accept a self-signed leaf cert through it, even when the fingerprint matches, and there is no public Node API to whitelist a specific leaf cert. For a self-signed leaf the bypass above is the only working method.

**On a HOSTED install with a CA-issued chain, `NODE_EXTRA_CA_CERTS` is the correct fix and the bypass is not.** A publicly-trusted host can still fail with `claude mcp list` reporting `Failed to connect - UNABLE_TO_GET_ISSUER_CERT` while system Node and the Windows stack both validate the same chain (`tls.connect` reports `authorized=true`, a plain `fetch` to `/admin/mcp` answers 200): the CA set the bundled runtime uses is the one that does not. Export the served chain's CA certs to a single PEM and point `NODE_EXTRA_CA_CERTS` at it with the same dual-set pattern above; measured, `claude mcp list` flips from `Failed to connect` to `Connected`. **Keep `NODE_TLS_REJECT_UNAUTHORIZED=0` off a hosted box**: these are servers, and the bypass disables cert validation for the whole Node process.
...
| A tool is refused on call with `Access denied ... Required permission: none. Allowed permission: none.` while sibling tools answer | **Check the name against `tools/list` first.** A name the server does not register answers this same denial, not "unknown tool": on MCP 0.6.0 the singular by-id getters for a product, a user and a user group, and the singular shipping getter, are not registered (the skills repo lists them under `notRegisteredOn060` in `scripts/mcp-tools/0.6.0.json`), and the plural list getters (`get_products_by_ids`, `get_users_by_ids`, `get_user_groups_by_ids`, `get_shipping_methods`) replace them [mcp 0.6.0]. The `Required permission` text is whatever the tool declares and says nothing about registration ([`backend-mcp-server.md`](../../dw-extend-mcp-tools/references/backend-mcp-server.md) "The identifier-parameter convention is split"). Only a name `tools/list` does list that is still refused is a grant question: the grant is per MCP configuration (its permission preset), not per client. Check the preset in the admin UI (Settings, Integration, MCP configurations) and re-create it with `Access = Full access` when it is narrower; until then fall back to the Management API read for that entity. |
| After a start, every friendly URL answers the DW 404 while `Default.aspx?ID=<id>` answers 200 (an empty shell), paragraph templates throw `NullReferenceException` on `Model.Item`, and no item-type duplicate warning was logged at that start | Frontend initialisation was skipped, not content lost: recycle the app pool once. See [`host-lifecycle.md`](host-lifecycle.md) "After any start". |
