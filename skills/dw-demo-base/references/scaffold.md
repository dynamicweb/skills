...
- **Install the Backend MCP AddIn** (the AppStore app **Truvio Commerce MCP**) **from the AppStore**. Never hand-write a `<PackageReference>` for it from memory — the pre-rename `Dynamicweb.MCP` id still restores cleanly and installs a stale AddIn. A csproj pin is an escape hatch that needs an explicit user choice after you report that the AppStore version could not be resolved. The AddIn is a non-skippable canonical step for every demo and registers at host startup. Recipe + auth model: [`../../dw-extend-mcp-tools/references/backend-mcp-server.md`](../../dw-extend-mcp-tools/references/backend-mcp-server.md) §1, then continue MCP configuration in `references/mcp-setup.md`.
...
1. **Create an empty database** first (`CREATE DATABASE [<demo-db>]` against `MSSQL$SQLEXPRESS`). Run it via `Invoke-Sqlcmd -TrustServerCertificate` (the self-signed local cert otherwise blocks the connect) — **not** `sqlcmd -E -i`, which current builds reject with a mutually-exclusive-flag error (`-E` integrated auth and `-i` input file collide). If you instead let the setup wizard create the DB (`CreateDatabase=true` on Step3), know the race: **"Create database" can report `Login failed` while the DB was in fact created** — re-POST Step3 (or re-click Next in the browser fallback) rather than treating the error as fatal; a `SELECT db_id('<demo-db>')` confirms it exists. Pre-creating with `Invoke-Sqlcmd` sidesteps the race entirely. Do **NOT** pre-provision `GlobalSettings.Database.config` against an empty DB — a pre-provisioned connection makes the wizard skip its own Step3 (the step that owns schema creation), and Step4 then faults with `Invalid object name 'AccessUser'`. DW10 does not migrate an empty DB on startup; with GlobalSettings pre-provisioned and an empty DB the wizard can never finish. **What decides "wizard or no wizard" is the DATABASE state, not the config**: a DB that already carries schema + an active Administrator row boots straight past the wizard.
   **Local installs only**: a hosted install is handed over already installed, so there is no database to create.
...
5. **Delete the leftover `Standard` area (AreaId 1).** Every wizard bootstrap seeds a `Standard` website that no demo uses; it must not survive into a demo host (it pollutes the area list and can grab root-URL resolution). Delete its pages, grid rows, paragraphs and the `Area` row via SQL — the MCP `delete_area` tool reports success without deleting ([`../../dw-extend-mcp-tools/references/backend-mcp-server.md`](../../dw-extend-mcp-tools/references/backend-mcp-server.md) §5). Verify `SELECT COUNT(*) FROM Area WHERE AreaId=1` returns 0.
   **Local installs only**: a hosted install is handed over already bootstrapped, and a leftover area there has no documented delete path (`delete_area` reports success without deleting), so an online build asks the user.
...
**Local installs only**: a hosted install arrives restored and configured, so this flavor does not apply there.

(The interactive Setup Guide in a browser still works as a fallback — same steps, ~20 minutes of clicking — but the HTTP recipe is the default. **Gotcha** either way: the license gate can leave every seeded user inactive with an empty password — no usable admin login. If `/admin` rejects every credential after setup, apply the headless admin-password recovery in [`../../dw-setup-install/references/install-anatomy.md`](../../dw-setup-install/references/install-anatomy.md) §8.)
...
The platform anti-patterns to avoid when scaffolding (don't target DW9, don't reference the bare `Dynamicweb` meta-package, don't use the CMS-only `dw10-cms` template) are owned by [`../../dw-setup-install/references/install-anatomy.md`](../../dw-setup-install/references/install-anatomy.md) §7.
...
**This pass owns the checkout.** The clone/pull below is the **only** one in a build, and the SHA it records in `CUSTOMISATIONS.md` is the build's reproducibility stamp. Every later pass (deserialize, layer activation, styles) **asserts** that SHA instead of pulling, so `main` cannot move under a half-finished deserialize — which matters because a machine may share one Distribution clone across demos. If a later pass finds a different SHA, it stops and reports "distribution checkout moved since scaffold, <recorded> -> <current>": re-run from this pass on the new SHA and record it, or reset the checkout to the recorded SHA. Never reconcile by continuing.
...
Cloning uses `git` (hence the setup-checks probe that `git` is present, plus `gh` authenticated so a private mirror clones over HTTPS via the gh credential helper). **The Distribution is the public repo `justdynamics/Truvio.Commerce.Distribution`**; the name is written into every clone snippet in this corpus, and `$env:DW_DISTRIBUTION_REPO` (owner/name form) is an optional override for a team that mirrors or forks it, never a prerequisite. An unset override is not a missing repo: clone the default. Never conclude "no Distribution access" from an unset variable or from a repo search under another org or name (one build did exactly that, fell through to path C of [branded-demo-paths.md](branded-demo-paths.md) and re-derived the catalogue, the personas and the sign-in by hand); if the default clone fails, report the git error verbatim and ask which mirror to use. The clone contains every live layer at the resolved commit; read whichever `layers/<name>/` dirs the edition composes. The versions prompt (DW10 + Swift version, asked before any artifact is fetched) is owned by `setup-checks.md`; the artifact table mapping each layer to its consumer stays in `SKILL.md` "Baseline data".
