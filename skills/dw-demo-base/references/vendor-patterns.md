...
> Sister skills cross-reference this file from their vendor-patterns section instead of restating it — single source for vendor positioning across all sister skills.
...
- PowerShell recipes fenced inside Markdown references, not Python scripts — Windows-native stack, no interpreter dependency.
- Composition routing ("Use FIRST" / "Use AFTER") instead of independent atomic skills — our use-case is a build pipeline; sisters loaded standalone silently no-op.
- Four surfaces (MCP / Management API / Direct SQL / Filesystem) instead of MCP + API only — bulk SQL fixes and `wwwroot/Files/` manipulation are unavoidable in real demo builds.
- Explicit inline recovery recipes instead of abstract "report transparently with retry options" — the recipes are load-bearing.
- Heavy-split layout (orchestrator SKILL.md + topic `references/`) instead of flat skill directories — reduces SKILL.md context cost, improves topic-targeted routing.
...
|---|---|---|
| **DC = AccessUser group** for any multi-DC B2B portal — unlocks DC-scoped Assortments, Shipping methods, and Shipping fees with no custom code. Naming convention: `DC-<code>`, with `AccessUserCustomerNumber` set to the same value. The standard B2B mechanic, not an upgrade path. | Dynamicweb vendor architect | [`foundational/commerce-b2b.md`](foundational/commerce-b2b.md) "The DC-as-user-group pattern" |
| **ERP-imported pre-graduated prices** for any qty-break / cart-time tier-price behavior. `EcomPrices.PriceQuantity > 0` tier rows are silently ignored by the stock cart resolver — the production pattern is one row per (product, user-group, qty-band) with the resolved price baked in, indexed via the same user-group mechanic as the DC pattern. | Dynamicweb vendor architect | [`foundational/commerce-catalog.md`](foundational/commerce-catalog.md) §2.11 "Pricing — tier rows are NOT honored by the stock cart" |
