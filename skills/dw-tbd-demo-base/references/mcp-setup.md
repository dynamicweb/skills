...
- **`count > 200`** â†’ gate passes; proceed to step 4 (drop guardrails) and then to the demo-type-specific path (PIM modelling via `dynamicweb-pim-demo`, or Swift baseline deserialize via [`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md)).
