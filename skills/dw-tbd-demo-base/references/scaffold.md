...
Suite version is whatever the template + `dotnet restore` resolve. **Version policy is out of scope for this skill** â€” neither pinning a specific patch nor enforcing Ring-N stability is something this skill does. If a particular demo needs a frozen version, edit its csproj directly.
...
- **.NET 10 SDK** is installed (`dotnet --list-sdks | Select-String '^10\.'` returns at least one row). Net 8 is NOT sufficient â€” see Section 2.1 below for why the host must run on net10.
- **`Dynamicweb.ProjectTemplates 1.26.0`** is installed (`dotnet new list | Select-String 'dw10-suite'` returns a match).
- **SQL Express service** (`MSSQL$SQLEXPRESS`) is running â€” needed by Section 3's first-run.
...
After the command completes, the solution folder contains a new `Dynamicweb.Host.Suite/` folder with the canonical Suite scaffold (`.csproj`, `Program.cs`, `Properties/launchSettings.json`, etc.). The csproj's `Dynamicweb.Suite` PackageReference is whatever the template ships with â€” leave it as-is unless the demo has a specific reason to freeze a version.
...
The MCP package (`Dynamicweb.MCP.0.2.0-BETA`) ships only `lib/net6.0/` and `lib/net8.0/` binaries (no `net10.0/`), so the *DLLs* themselves load fine on net8 â€” but the AddIn loader's runtime check fails. Symptom: install POST `/Admin/Api/AddinInstall` returns 200, files drop to `wwwroot/Files/System/AddIns/Installed/Dynamicweb.MCP.<ver>/lib/`, but the AddIn never registers, never appears in Installed Apps, and `/admin/mcp` returns 404. Indistinguishable from the queue-stuck DB-update bug (`references/db-update-recovery.md`).

**Triage in this exact order:**

1. Open the host startup log. If it says `Dynamicweb is running on .NET 8`, **the TFM is wrong** â€” fix this first. Edit csproj, restart, verify the log now says `Dynamicweb is running on .NET 10 or greater`.
...
### 2.1b â€” Patch `Program.cs` for .NET 7+ distributed-transactions opt-in (mandatory)
...
This setting is per-process and applies for the lifetime. Setting it in `Program.cs` ahead of any DI / framework init is the safest place â€” it gets installed before any TransactionScope is opened.

This is a separate prereq from the MSDTC + firewall configuration documented in `references/setup-checks.md` Â§"MSDTC for cross-connection TransactionScope". Both are required and orthogonal:

- **MSDTC inbound/outbound + firewall** â†’ makes the *transport* available
...
`wwwroot\Files\System\` is a runtime-managed folder (AddIns, Indexes, Repositories, Log, Diagnostics, etc.). It is created and mutated by the host at runtime â€” none of it is source. The Web SDK's default Content glob pulls everything under `wwwroot/**` into MSBuild's item set, so a populated demo ends up tracking thousands of runtime files on every build (typical: 7k+ files / 20+ MB after first run, growing with use). MSBuild's up-to-date check then walks all of them, slowing both incremental and full builds.
...
Apply this at scaffold time, before the first `dotnet run` â€” the glob safely matches nothing until the host populates the folder, so the exclusion is in place from day one. The host reads/writes `wwwroot\Files\System\` at runtime regardless of MSBuild item membership; excluding it from the build does not change runtime behaviour.

### 2.3 â€” Release rings (regression triage only)

Dynamicweb publishes the Suite as five parallel NuGet packages â€” one per release ring. Ring 0 (`Dynamicweb.Suite`) is the stable default and the ring every demo ships on. Rings 1â€“4 (`Dynamicweb.Suite.Ring1` â€¦ `Ring4`) are earlier-cadence preview tracks â€” higher number = earlier ring, faster cadence.
...
| `Dynamicweb.Suite.Ring2..4` | 2â€“4 | `YYYY.M.D` | Internal validation tracks; rarely useful for demos |

Browse versions at <https://www.nuget.org/packages?q=DynamicWeb.Suite>. Mind the version-scheme split: Ring 0 uses semver-style `10.x.y`; Rings 1â€“4 use date-stamped `YYYY.M.D`. A `10.*` float will NOT match a Ring-N package, and vice versa.

**When to swap.** Only as a regression-triage tool: a bug shows on Ring 0 and you want to know whether it reproduces on a later ring. Same result on Ring N â†’ platform-wide bug; gone on Ring N â†’ ring-specific, will land in the next Ring-0 promotion. After triage, **swap back to `Dynamicweb.Suite`** before continuing the demo build. Do not ship a demo on a non-stable ring.

**Swap recipe** â€” edit the csproj:
...
**Out of scope here.** The skill defaults the host to Ring 0 and never auto-pins a ring. Section 2.3 surfaces the swap only as a debugging tool â€” the disclaimer in this file's intro still holds: "Suite version is whatever the template + `dotnet restore` resolve".

---

## 3. First run â€” Setup Guide for DB + Files folder
...
3. Print a one-time admin user prompt (capture the credentials â€” they're needed for the admin UI walkthrough in `references/mcp-setup.md` Step 3).
...
After the first run completes, the per-demo project files are the source of truth â€” sister skills must read from these, not from hardcoded fallbacks:
...
| Database name | `Dynamicweb.Host.Suite/GlobalSettings.Database.config` (`Database=` or `Initial Catalog=` in connection string) | Swift's [`../../dw-tbd-swift-demo/references/integrity-sweep.md`](../../dw-tbd-swift-demo/references/integrity-sweep.md) SQL probes; PIM admin/SQL recipes |
| Management API bearer token | Captured via `AskUserQuestion` from chat (format `CLAUDE.<hex>`). Storage contract is canonical in `references/mcp-setup.md` Step 6. | Swift's [`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md) + [`../../dw-tbd-swift-demo/references/integrity-sweep.md`](../../dw-tbd-swift-demo/references/integrity-sweep.md); any Management API call |
