...
>
> **Operational baseline-deserialize steps** (POST `/Admin/Api/SerializerDeserialize`, integrity sweep, schema-drift workarounds) are owned by [`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md). Only Swift demos need that flow â€” PIM demos start from a blank/fresh DB.
>
...
For Swift baseline restore ([`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md)), only Deploy mode is used. Seed mode is out of scope for the canonical Swift baseline-load flow.
...
The vault baseline at `$env:DW_VAULT\serialized-data\Swift2.2\` is **content-only** (as of 2026-05-08 â€” the historical `_sql/` framework rows were deliberately removed: they silently overwrote framework data hosts had already built via the PIM-skill flow). One top-level subfolder: `_content/`, a mirror tree of the DW areaâ†’pageâ†’gridRowâ†’paragraph hierarchy, one YAML file per node (folder = page; files = `area.yml`, `page.yml`, `grid-row.yml`, `paragraph-<col>-<n>.yml`). Hosts that need a baseline framework should run [`../../dw-tbd-pim-demo/references/canonical-setup-order.md`](../../dw-tbd-pim-demo/references/canonical-setup-order.md) Steps 1-4 before this deserialize. The runtime contract is [`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md) Â§3 "Baseline shape".
...
| Run a baseline content deserialize (Swift demos only) | [`../../dw-tbd-swift-demo/references/deserialize-flow.md`](../../dw-tbd-swift-demo/references/deserialize-flow.md) |
| Post-deserialize integrity checks | [`../../dw-tbd-swift-demo/references/integrity-sweep.md`](../../dw-tbd-swift-demo/references/integrity-sweep.md) |
