...
# Note: no `_sql/` to copy — current baseline is content-only by design (2026-05-08).
...
Superseded 2026-05-08: the baseline ships no `_sql/`, so the former `EcomCurrencies.CurrencyUseCurrencyCodeForFormat` column-strip workaround is obsolete — reconstruct from git history if a future baseline reintroduces `_sql/`.
...
Superseded 2026-05-08: deploy-design-first is the only viable path — the former Approach A ("strip Content predicates") no longer applies with a content-only baseline, and running with strict mode off remains forbidden per §4.
