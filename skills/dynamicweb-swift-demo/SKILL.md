...
description: Truvio (Dynamicweb 10) Swift frontend -- owns the baseline content deserialize (Swift2.2 vault content via DynamicWeb.Serializer strict-mode POST + mandatory post-deserialize integrity sweep), templates, paragraph types, Visual Editor, asset organisation, the customer-center / sales-on-behalf playbook (stock CSR section -- never rebuild it), and the customer re-skin escalation ladder. Triggers: starting a Swift demo (load the baseline), re-skinning to a customer brand, "where do I edit the header/footer", customer-center / impersonation flows, paragraph renders empty or stale, Razor pitfalls in custom layouts, language layers, gating pages or paragraphs by group, SQL-direct content seeding. Swift 2 only -- never follow `doc.dynamicweb.dev/swift/swift-1/` URLs. Use AFTER truvio-demo-base (host running, Serializer installed).
---

# Truvio Swift Demo Skill

Baseline content deserialize, frontend / Swift / customer-center playbook, and re-skin recipe for Truvio (Dynamicweb 10) demo builds. **Use AFTER** `truvio-demo-base` -- assumes a host is running, `$env:DW_VAULT` resolves, and the Serializer is installed in the host (per base's [`../truvio-demo-base/references/serializer-reference.md`](../truvio-demo-base/references/serializer-reference.md) "Installation").
...
2. **Verify Serializer is installed** in the host per [`../truvio-demo-base/references/serializer-reference.md`](../truvio-demo-base/references/serializer-reference.md) "Installation".
...
Inherited principle from [`../truvio-demo-base/SKILL.md` "Demo philosophy"](../truvio-demo-base/SKILL.md). Swift-specific guardrails:
...
- **Product catalogue: deep AND wide — exception case.** Rich product data is welcome on the storefront. PIM owns the modelling depth; Swift just renders it. See `truvio-pim-demo` for the modelling recipes.
...
| Add a language layer to a website (sibling `Area` rows, language management settings, OOTB `Swift-v2_LanguageSelector` wiring; sister doc is `truvio-pim-demo/references/localization.md` for the PIM/product side) | references/language-layers.md |
...
| Customer-context read-only contract | [truvio-demo-base/references/customer-context.md](../truvio-demo-base/references/customer-context.md) |
| Customisations-ledger preflight | [truvio-demo-base/references/customisations.md](../truvio-demo-base/references/customisations.md) |
| Baseline-drift self-diagnosis rule | [truvio-demo-base/SKILL.md "Self-diagnosis rule"](../truvio-demo-base/SKILL.md) |

Runtime enforcement: the per-demo `CLAUDE.md` drop installed by base (`truvio-demo-base/references/customer-context.md` recipe) reminds Claude of these rules regardless of which skill loaded first.

If you find yourself running this skill standalone with no base context, fix that before continuing -- see the description's "Use AFTER" hint. If `~/.claude/skills/truvio-demo-base/` is not installed, install it first -- this skill's inherited rules require it.

## Sister skills

- **`truvio-demo-base`** -- foundation skill (Use FIRST). Owns all setup + path resolution + Serializer install + customisations + customer-context. Does NOT deserialize a baseline — that's owned here.
- **`truvio-pim-demo`** -- PIM modelling (Use AFTER, can pair with this skill in either order on the host). Starts from a blank/fresh DB and skips the baseline deserialize entirely.
...
The vendor skill-repo consultation outcome (`dynamicweb/skills` + `dynamicweb/ai-implementor-skills`) is documented in [truvio-demo-base/references/vendor-patterns.md](../truvio-demo-base/references/vendor-patterns.md) -- the same shared note covers both PIM and Swift sister skills.
