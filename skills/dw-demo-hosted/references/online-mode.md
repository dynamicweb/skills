...
>
> Local installs remain the default; this reference is the fork target from [`dw-demo-base` SKILL.md](../../dw-demo-base/SKILL.md) "Environment fork". The four always-on guardrails (customisations ledger, customer-context read-only, demo philosophy, discover-from-project-files) apply unchanged.
...
3. **Real restart** — drop a control file in `Files/System/CloudHosting/` (`recycle.txt`, `restart.txt`; `changeversion.txt` also switches release ring). Canonical table: [`dw-setup-config`](../../dw-setup-config/SKILL.md) "cloud control files". Some **global settings do not take effect on a cache flush alone** — a URL-generation change can keep serving the old shape until the app restarts. If a setting reads back correctly from the API but the storefront still renders the old behaviour, restart before diagnosing further. **A CHANGED token is what switches the release ring — a same-value re-upload is NOT a reliable no-op.** Observed on an `R0-NET…` ring token: re-uploading `changeversion.txt` with the value it already holds still recycles the app but leaves the version where it was. Two consequences: (a) to actually switch ring/version, write a token that DIFFERS from the file's current content — a recycle alone is not proof the switch happened, confirm with `info.version`; (b) never re-upload the current value assuming nothing will happen — you can trigger an unintended recycle. Record the last-used token in the demo ledger so the next switch bumps past it rather than re-writing it.
