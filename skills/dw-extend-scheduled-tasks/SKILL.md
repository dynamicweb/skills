...
> **Demo / content-editing scope guard.** Do **not** use `RunSqlScheduledTaskAddIn` (or any SQL-via-scheduled-task motion) to create or edit demo/content data because the API "got hard". The admin UI is API-first — every UI action lands on `/Admin/Api`, so if the UI can do it an endpoint exists: capture the SPA's network call and replay it (MCP → Management API), and file a learning if a surface genuinely seems missing. (The one sanctioned scheduled-task-SQL use is a deliberate between-demo state-reset fixture — a state reset, not a content-authoring escape hatch; the demo skills document it where it applies.)
