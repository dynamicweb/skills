...
- **Mock flavors** (this skill): no CUSTOM provider class is registered — the shipped `SqlProvider` / `EcomProvider` / `OrderProvider` do the work, so an activity over them is configuration. The [two-directional flavor](two-way-mock.md) runs four such activities and shows the round trip on camera. The one-direction flavor ([mock-deltas.md](mock-deltas.md)) goes further and stages the result: the database is pre-staged into the **post-BC-sync state** — every value BC would have written is already in `EcomProducts`, as if the delta arrived overnight — and a RESET task flips it back between demos — bound to a restore activity where the reset can be expressed as one, since `RunSqlScheduledTaskAddIn` reports success whether or not it executed anything ([mock-deltas.md](mock-deltas.md) "Three ways to execute the mock"). The demo narrates "BC sent us this; look at the result", with the data + action-rule definition + email template as evidence. The PIM→BC direction is told via one static field-mapping artefact. The framework concepts (source provider, destination provider, activity, field mapping) are narrated against that staged state, not against live wires. (Superseded: an earlier version of this flavor used inbox/outbox JSON files — see the "Do not" section of mock-deltas.md.)
...
- Two-directional mock (order out, document number back): [two-way-mock.md](two-way-mock.md).
- Job-file format, provider behaviour and the surface ladder for activities: [`../../dw-integration-framework/SKILL.md`](../../dw-integration-framework/SKILL.md).
