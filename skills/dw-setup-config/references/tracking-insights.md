# Tracking, Insights and health providers

> Read this before promising anyone *any* number on an Insights dashboard. The Marketing tiles read a
...
3. Put step 2 on the **bring-up checklist for every hosted/proxied host** — it is a precondition, not a
   fix. On an ACL-locked host both settings need a two-step recipe: apply the setting through the admin
   surface, then persist it so it survives a recycle (an ACL-locked `Files/` tree splits GlobalSettings
   apply from persist).
...
- **A solution that needs a coherent Marketing dashboard on this version must generate its session/view rows directly** rather
...
`2`. Assert the parse rather than the string before making any claim about bot/device filtering.
...
`TableTimeInterval = None` (a common setting) the live names are the plain ones — so **any sibling
...
"re-fix" a task on the strength of that column. (Task creation semantics worth knowing when scripting tasks
over the Management API: a daily task re-anchors its schedule on save, `TaskToggleActive` toggles rather than
sets, and a save with `Id=0` creates a new task on every call instead of upserting.)
...
`UnauthorizedAccessException`. So a site accumulates rows for its entire life. An inherited long-lived host
can open its Monitoring dashboard on hundreds of errors and thousands of events in 24h, off tables holding
six figures of rows going back more than a year.

Any host whose Monitoring dashboard is shown **from the admin UI** needs an explicit daily log-clear task:
...
their backing queries" section of `management-api-and-sql.md` in [`dw-data-access`](../../dw-data-access/SKILL.md).

## Cross-references

- `management-api-and-sql.md` in [`dw-data-access`](../../dw-data-access/SKILL.md) — the Management API
  surface, verb discovery, and the cost of guessing verb names against an owner-visible dashboard.
