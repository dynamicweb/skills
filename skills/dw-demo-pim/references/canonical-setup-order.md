...
14. **Variants** — attach groups + insert combinations + INSERT variant product rows. Two routes, picked by install type — **on a hosted/API-only install (no SQL), use the Management API chain** in [`commerce-catalog.md`](../../dw-demo-base/references/foundational/commerce-catalog.md) §2.14: `VariantCombinationSave` (with the reused setup cache key) creates the per-variant rows, a full-model round-trip `ProductById?Id&VariantId` → `ProductSave` writes per-variant number/active/stock, and `PriceSave{VariantId}` writes the price. Exercise that chain verbatim before concluding a variant row is unwritable — the verbs outside it report success and write nothing (catalogue in §2.14). **On a local install, the SQL sweep is the canonical variant-enrichment step:** `patch_products_safe` against a variant row (`id` + `variantId`) reports success and echoes the requested values while the variant `EcomProducts` row stays untouched — the echo is the input model, not a post-write read (silent no-op catalogue, [`extend-mcp-tools.md`](../../dw-demo-base/references/foundational/extend-mcp-tools.md) §5). Never trust a tool echo for variants; `SELECT` the variant row immediately after any tool-based variant write. Each variant `EcomProducts` row MUST have a unique `ProductNumber` (master + dash + variant-option suffix); collisions silently break the PIM-for-BC connector import. Master renames can re-sync variant numbers — see `structural-model.md` §2.5 for the rule, the regression vector, and the idempotent re-suffix UPDATE.
