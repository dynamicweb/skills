...
- **MCP `save_*` / `create_or_update_*` / admin-UI Visual Editor / admin-UI form save** â†’ these go through DW's domain services, which invalidate caches inline. You do not consult this table; you do not restart the host. The surface-priority table in [`dynamicweb-demo-base/SKILL.md` "Surface priority for CREATES"](../../dw-demo-base/SKILL.md) names these as the default for any structural CREATE for exactly this reason.
- **Management API** (`POST /admin/api/...`) â†’ same domain services as MCP; same cache invalidation behavior. The dedicated `CacheInformationRefresh` / `BuildIndex` / `GetServiceCaches` endpoints listed below are themselves Management API calls â€” use them when you need to flush something explicitly without restarting. On a **hosted install** there is no restart at all: every "YES restart" row below resolves to a bulk flush instead â€” `GetServiceCaches` â†’ `CacheInformationsRefresh {"Ids": [...]}` (plural, takes the service ids); recipe in [`dynamicweb-demo-base/references/online-mode.md`](../../dw-demo-base/references/online-mode.md) Â§"Cache refresh = the online host restart".
