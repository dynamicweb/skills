...
| Management API | `PageSave` with `publicationState: "Hidden"` (or `"hidden"`) | 500 `Exception has been thrown by the target of an invocation.`, nothing written | `Hidden` is no member of the save enum: `PublishingMode` is `Published`, `Unpublished`, `HideInMenu` [dw 10.28.11]. The read model returns lower case, so its casing is not the save enum's casing. | To keep a page live but out of the navigation, send `HideInMenu` and confirm with an anonymous GET (200) and the rendered menu (page absent). To take a page off the site, send `Unpublished` (it then answers 404 anonymously) and confirm with an anonymous GET. |
