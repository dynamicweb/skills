...
| 500 `Exception has been thrown by the target of an invocation.` on an enum value you set | The value is not a member name in the save enum's casing: the read model returns another casing (`publicationState` reads `published` and saves `Unpublished`), and a natural value such as `Hidden` is no member at all (a page's `PublishingMode` is `Published`, `Unpublished`, `HideInMenu`). | Send the member name in the save enum's casing (`HideInMenu` to hide a page from menus); this 500 is a value error, never a host fault. | [`recipes-content.md`](recipes-content.md) "Page verb and tool traps" |
